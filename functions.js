...
function composeSkillsAndAddons(skills, addons) {
  ...
  return skills.reduce((skillsAndAddons, value) => {
    ...
    skillsAndAddons[value.category].skills =
      skillsAndAddons[value.category].skills || [];
    ...
    if (value.has_addons && !skillsAndAddons[value.category].addons) {
      ...
      skillsAndAddons[CATEGORY_NAME].addons =
        skillsAndAddons[CATEGORY_NAME].addons || [];

      const addonsWithMatchingCategory = addons.filter(
        addon => addon.category === CATEGORY_NAME
      );
